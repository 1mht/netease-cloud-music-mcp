...
def get_song_detail_by_id(song_id):
    ...
    try:
        ...
        item = data['songs'][0]
        artists_details = [{'id': str(a.get('id')), 'name': a.get('name')} for a in item.get('artists', [])]
...
def search_songs(keyword, limit=10, offset=0):
    ...
    try:
        ...
        if data.get('code') != 200 or 'result' not in data or 'songs' not in data['result']:
            # 尝试返回空列表而不是报错
            return []
        ...
        try:
            ...
            if song_ids:
                ...
                detail_params = {'id': song_ids[0], 'ids': ids_param} # id 参数随便传一个，ids 才是关键
        ...
        for item in search_songs_list:
            ...
            song_info = {
                ...
                'album': item.get('album', {}).get('name'),
                'album_id': item.get('album', {}).get('id'),
                'album_pic_url': item.get('album', {}).get('picUrl'), # 详情接口里肯定有这个
                'duration_ms': item.get('duration'),
                'publish_time': item.get('album', {}).get('publishTime')
            }
...
def interactive_select_song(initial_keyword):
    ...
    while True:
        ...
        for i, song in enumerate(songs):
            ...
            name = (song['name'][:22] + '..') if len(song['name']) > 24 else song['name']
            artists = (artists_str[:18] + '..') if len(artists_str) > 18 else artists_str
            album = (song['album'][:18] + '..') if song['album'] and len(song['album']) > 18 else (song['album'] or "未知")
