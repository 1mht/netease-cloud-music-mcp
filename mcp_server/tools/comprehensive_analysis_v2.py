...
def analyze_comments_v2(
    ...
    try:
        ...
        from mcp_server.tools.sampling_decision import smart_sampling_decision, execute_smart_sampling
        ...
        if auto_sample and triggered:
            try:
                from mcp_server.tools.sampling_v5 import sample_comments_v5, SamplingConfig
                ...
                if sample_result:
                    ...
                    coverage_rate = (actual_sampled / api_total * 100) if api_total > 0 else 0
                    ...
                    logger.info(f"[采样] 完成! 新增{sample_result.get('samples_saved')}条")
        ...
        # ===== 5. 构建元信息 =====
        timestamps = [getattr(c, 'timestamp', 0) or 0 for c in comments if getattr(c, 'timestamp', 0)]
        if timestamps:
            min_ts, max_ts = min(timestamps), max(timestamps)
            earliest = datetime.fromtimestamp(min_ts / 1000).strftime("%Y-%m-%d") if min_ts > 0 else "unknown"
            latest = datetime.fromtimestamp(max_ts / 1000).strftime("%Y-%m-%d") if max_ts > 0 else "unknown"
            years_covered = round((max_ts - min_ts) / (1000 * 60 * 60 * 24 * 365), 1) if min_ts > 0 else 0
        else:
        ...
        if include_dimensions:
            dimensions_result = {
                k: v for k, v in dimensions_result.items()
                if k in include_dimensions
            }
        ...
        step1_data_overview = {
            ...
            "ai_note": "覆盖率<1%时结论需谨慎，采样可能有偏差" if api_total > 0 and db_count/api_total < 0.01 else "数据量可接受"
        }
        ...
        if anchor_contrast_samples:
            step3_verification_samples["anchors"] = anchor_contrast_samples.get("anchors", {})
            step3_verification_samples["contrast"] = anchor_contrast_samples.get("contrast", {})
...
def _extract_cross_dimension_signals(dimensions: Dict[str, dict]) -> List[str]:
    ...
    short_ratio = ling_qf.get("metrics", {}).get("type_distribution", {}).get("Short", 0)
    ...
    for dim_id, dim_result in dimensions.items():
        ...
        if sample_size < 50:
            signals.append(
                f"{dim_id}维度样本量仅{sample_size}条，结果置信度较低"
            )
...
def get_raw_comments_v2(
    ...
    try:
        ...
        comments = query.order_by(Comment.liked_count.desc()).limit(limit * 3).all()
        ...
        if year:
            ...
            for c in comments:
                ts = getattr(c, 'timestamp', 0) or 0
                if ts > 0:
                    c_year = datetime.fromtimestamp(ts / 1000).year
                    if c_year == year:
                        filtered.append(c)
