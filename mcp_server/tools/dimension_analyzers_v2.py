...
def analyze_sentiment_v2(comments: List[Any]) -> dict:
    ...
    normal_mean = sum(normal_scores) / len(normal_scores) if normal_scores else mean_score
    ...
    samples_high = sorted([s for s in all_samples if s["algo_score"] >= 0.8],
                          key=lambda x: -x["algo_score"])[:5]
    samples_low = sorted([s for s in all_samples if s["algo_score"] <= 0.2],
                         key=lambda x: x["algo_score"])[:5]
    ...
    return {
        ...
        "quantified_facts": {
            ...
            "statistics": {
                ...
                "hot_low_score_ratio": round(hot_low_score_ratio, 3),  # v0.7.7: 高赞中低分占比
...
def analyze_content_v2(comments: List[Any]) -> dict:
    ...
    texts = [getattr(c, 'content', '') or '' for c in comments if getattr(c, 'content', '')]
    if not texts:
        return _empty_result("content", "内容分析")
    ...
    try:
        tags = jieba.analyse.extract_tags(
            full_text, topK=20, withWeight=True,
    ...
    if themes and themes[0]["percentage"] > 0.5:
        signals.append(f"'{themes[0]['name']}'主题匹配度{int(themes[0]['percentage']*100)}%")

    # 检测人名/事件关键词
    for kw in keywords[:10]:
        if len(kw["word"]) >= 2 and kw["weight"] > 0.1:
            signals.append(f"高频词'{kw['word']}'({int(kw['weight']*100)}%)值得关注")
            break
...
def _classify_themes_v2(keywords: List[dict], texts: List[str]) -> List[dict]:
    ...
    for theme, words in theme_rules.items():
        matches = sum(1 for w in words if w in keyword_words or any(w in t for t in texts[:100]))
...
def analyze_temporal_v2(comments: List[Any]) -> dict:
    ...
    # 信号1：时间跨度
    if time_span >= 5:
        signals.append(f"评论时间跨度{time_span}年（{first_year}-{last_year}），可分析长期趋势")
    elif time_span <= 2:
...
def analyze_structural_v2(comments: List[Any]) -> dict:
    ...
    if hot_lengths:
        ...
        hot_analysis = {
            ...
            "mean_length": round(sum(hot_lengths) / hot_total, 1) if hot_total > 0 else 0,
    ...
    if hot_analysis:
        ...
        if hot_analysis["mean_length"] > 50:
            signals.append(
                f"热评平均{hot_analysis['mean_length']:.0f}字，内容较为丰富"
            )
...
def analyze_social_v2(comments: List[Any]) -> dict:
    ...
    # 热评样本
    hot_samples = []
    for c in sorted(comments, key=lambda x: getattr(x, 'liked_count', 0) or 0, reverse=True)[:10]:
        hot_samples.append({
            "id": str(getattr(c, 'comment_id', '')),
            "content": (getattr(c, 'content', '') or '')[:100],
            "likes": getattr(c, 'liked_count', 0) or 0,
...
def _classify_comment_type(content: str) -> str:
    ...
    music_terms = {'编曲', '作词', '作曲', '音色', '吉他', '贝斯', '混音', '前奏', '和声', '唱功', '旋律', '歌词', '副歌'}
    if sum(1 for t in music_terms if t in content) >= 2:
        return "Review"

    story_kw = {'记得', '那年', '那时', '当时', '曾经', '后来', '故事', '第一次', '小时候', '以前'}
...
def analyze_all_dimensions_v2(
    comments: List[Any],
    include_anchor_contrast: bool = True
) -> Dict[str, Any]:
    """
    ...
    if include_anchor_contrast and comments:
        try:
            # 修复导入路径：使用相对导入
            from mcp_server.tools.sample_selector import select_anchor_and_contrast_samples
            ...
            logger.info(f"锚点/对比样本生成成功: anchors={len(anchor_contrast.get('anchors', {}).get('most_liked', []))}")
